...
Usage:
    ...
    uv run python scripts/python/scrape/twikit/enrich_twikit_profiles.py --role apoyo \\
      --top-supporters 30 --max-follows 2000 --ingest

    uv run python scripts/python/scrape/twikit/enrich_twikit_profiles.py \\
...
import db  # noqa: E402
from scrape.twikit.scrape_twikit_twitter import (  # noqa: E402
    DEFAULT_COOKIES,
    TwikitScrapeError,
    _login_client,
    _tweet_to_dict,
...
def _load_top_haters(n: int, target_username: str | None = None) -> list[str]:
    """Rank haters by hostile reply count; optionally scope to one target account."""
    ...
    try:
        if target_username:
            ...
            rows = con.execute(
                ...
                WHERE LOWER(TRIM(t.username)) = ?
                  AND r.username IS NOT NULL
                  AND LENGTH(TRIM(r.username)) > 0
                  AND COALESCE(c.criterio_label, '') = 'derecha_o_troll'
                GROUP BY r.username
                ORDER BY COUNT(*) DESC, MAX(r.created_at_ts) DESC NULLS LAST
                LIMIT ?
                """,
                [target, n],
            ).fetchall()
...
def _load_top_supporters(n: int, target_username: str | None = None) -> list[str]:
    """Rank supporters by apoyo reply count; optionally scope to one target account."""
    con = db.connect(read_only=True)
    try:
        if target_username:
            target = target_username.lstrip("@").lower()
            rows = con.execute(
                ...
                WHERE LOWER(TRIM(t.username)) = ?
                  AND r.username IS NOT NULL
                  AND LENGTH(TRIM(r.username)) > 0
                  AND COALESCE(c.criterio_label, '') = 'apoyo_izquierda'
                GROUP BY r.username
                ORDER BY COUNT(*) DESC, MAX(r.created_at_ts) DESC NULLS LAST
                LIMIT ?
                """,
...
def _run_ingest(run_dirs: list[Path], *, role: str = "hater") -> None:
    ...
    sql_path = db.resolve_sql(sql_name)
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Scope --top-haters / --top-supporters to replies on this target account",
    )
    ...
    target = (args.target_username or "").lstrip("@") or None
    ...
    if args.handles:
        ...
        if args.top_supporters < 1:
            ...
            return 1
        handles = _load_top_supporters(args.top_supporters, target_username=target)
        scope = f" on @{target}" if target else ""
        print(f"Top {args.top_supporters} supporters{scope}: {', '.join(handles)}")
    elif args.top_haters is not None:
        if args.top_haters < 1:
            print("--top-haters must be >= 1", file=sys.stderr)
            return 1
        handles = _load_top_haters(args.top_haters, target_username=target)
        scope = f" on @{target}" if target else ""
        print(f"Top {args.top_haters} haters{scope}: {', '.join(handles)}")
    else:
        print(
