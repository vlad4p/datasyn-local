...
-- ---------------------------------------------------------------------------
-- Haters top por persona (FB trolls top 10 + TW hostiles top 75)
-- ---------------------------------------------------------------------------
...
tw_agg AS (
    SELECT
        LOWER(TRIM(t.username)) AS target_handle,
        CAST(r.author_id AS VARCHAR) AS author_id,
        MAX(r.username) AS username,
        MAX(r.author_name) AS author_name,
        COUNT(*) AS hater_replies
    FROM silver.tk_tw_reply AS r
    JOIN silver.tk_tw_tweet AS t
        ON t.tweet_id = r.parent_tweet_id
    JOIN silver.tk_tw_reply_classification AS c
        ON c.reply_id = r.reply_id
    WHERE c.criterio_label = 'derecha_o_troll'
      AND t.username IS NOT NULL
      AND r.author_id IS NOT NULL
    GROUP BY LOWER(TRIM(t.username)), CAST(r.author_id AS VARCHAR)
),
tw AS (
    SELECT
        m.persona_id,
        m.nombre_canonico,
        'twitter'::VARCHAR AS plataforma,
        COALESCE(b.user_id, a.author_id) AS actor_id,
        COALESCE(b.username, a.username, a.author_name) AS actor_nombre,
        a.hater_replies AS score_eventos,
        CAST(NULL AS BIGINT) AS dias_activos,
        CAST(m.handle AS VARCHAR) AS cuentas_objetivo,
        CAST(b.narrativas AS VARCHAR) AS narrativas,
        b.tier,
        CAST(b.risk_score AS DOUBLE) AS risk_score,
        ROW_NUMBER() OVER (
            PARTITION BY m.persona_id
            ORDER BY a.hater_replies DESC, COALESCE(b.risk_score, 0) DESC
        ) AS ranking_src
    FROM tw_agg AS a
    JOIN gold.v_monitor_cuenta_map AS m
        ON m.plataforma = 'twitter'
       AND LOWER(TRIM(m.handle)) = a.target_handle
    LEFT JOIN gold.tk_troll_blacklist AS b
        ON CAST(b.user_id AS VARCHAR) = a.author_id
        OR LOWER(TRIM(b.username)) = LOWER(TRIM(a.username))
),
unioned AS (
    SELECT * FROM fb
    UNION ALL
    SELECT * FROM tw WHERE ranking_src <= 75
),
ranked AS (
    SELECT
        *,
...
SELECT *
FROM ranked
WHERE (plataforma = 'facebook' AND ranking <= 10)
   OR (plataforma = 'twitter' AND ranking <= 75);

-- ---------------------------------------------------------------------------
-- Apoyo / defensores top 50 por persona (TW supporters, scoped por target)
-- ---------------------------------------------------------------------------
...
WITH tw_agg AS (
    SELECT
        ...
        MAX(r.username) AS username,
        MAX(r.author_name) AS author_name,
        COUNT(*) AS apoyo_replies
    FROM silver.tk_tw_reply AS r
    JOIN silver.tk_tw_tweet AS t
        ON t.tweet_id = r.parent_tweet_id
    JOIN silver.tk_tw_reply_classification AS c
        ON c.reply_id = r.reply_id
    WHERE c.criterio_label = 'apoyo_izquierda'
      AND t.username IS NOT NULL
      AND r.author_id IS NOT NULL
    GROUP BY LOWER(TRIM(t.username)), CAST(r.author_id AS VARCHAR)
),
tw AS (
    SELECT
        ...
        COALESCE(a.username, a.author_name) AS actor_nombre,
        a.apoyo_replies AS score_eventos,
        CAST(NULL AS BIGINT) AS dias_activos,
        CAST(m.handle AS VARCHAR) AS cuentas_objetivo,
        ...
        ROW_NUMBER() OVER (
            PARTITION BY m.persona_id
            ORDER BY a.apoyo_replies DESC, COALESCE(risk.risk_score, 0) DESC
        ) AS ranking_src
    FROM tw_agg AS a
    JOIN gold.v_monitor_cuenta_map AS m
        ON m.plataforma = 'twitter'
    ...
    LEFT JOIN gold.tk_apoyo_profile_risk AS risk
        ON CAST(risk.user_id AS VARCHAR) = a.author_id
),
ranked AS (
    SELECT
        *,
...
SELECT
    ...
    m.nombre_canonico,
    'twitter'::VARCHAR AS plataforma,
    COALESCE(c.label, 'sin_cluster') AS narrativa,
    'derecha_o_troll'::VARCHAR AS posicion,
    COUNT(a.reply_id) AS comentarios,
    ROUND(
        100.0 * COUNT(a.reply_id)
        / NULLIF(SUM(COUNT(a.reply_id)) OVER (PARTITION BY m.persona_id), 0),
        2
    ) AS pct_narrativa
FROM gold.tk_hater_narrativa_assignment AS a
JOIN gold.tk_hater_narrativa_cluster AS c
    ON c.cluster_id = a.cluster_id
JOIN silver.tk_tw_tweet AS t
    ON t.tweet_id = a.parent_tweet_id
JOIN gold.v_monitor_cuenta_map AS m
    ON m.plataforma = 'twitter'
   AND LOWER(TRIM(m.handle)) = LOWER(TRIM(t.username))
GROUP BY m.persona_id, m.nombre_canonico, COALESCE(c.label, 'sin_cluster')
...
JOIN gold.tk_apoyo_narrativa_cluster AS c
    ON c.cluster_id = a.cluster_id
JOIN silver.tk_tw_tweet AS t
    ON t.tweet_id = a.parent_tweet_id
JOIN gold.v_monitor_cuenta_map AS m
...
-- TW hater narrativa temporal (as hostile signal) — scoped by target tweet author
SELECT
    m.persona_id,
    m.nombre_canonico,
    'twitter'::VARCHAR AS plataforma,
...
GROUP BY m.persona_id, m.nombre_canonico, DATE_TRUNC('day', r.created_at_ts)::DATE, c.label

UNION ALL

-- TW apoyo narrativa temporal (as support signal) — scoped by target tweet author
SELECT
    m.persona_id,
    m.nombre_canonico,
    'twitter'::VARCHAR AS plataforma,
