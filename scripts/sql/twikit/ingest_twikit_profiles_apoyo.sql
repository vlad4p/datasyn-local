-- Twikit APOYO (supporter) profile enrich → bronze + silver
-- Paths filled by enrich_twikit_profiles.py --role apoyo --ingest from
-- data/landing/redes/twikit/profiles/apoyo/_current_batch_apoyo.json
-- Separate tables so hater ingest (profiles/*/) is not mixed.
...
CREATE OR REPLACE TABLE bronze.tk_tw_profile_enriched_apoyo AS
...
CREATE OR REPLACE TABLE bronze.tk_tw_profile_post_apoyo AS
SELECT * FROM read_json_auto(
  '{{posts_glob}}',
...
CREATE OR REPLACE TABLE bronze.tk_tw_follow_edge_raw_apoyo AS
SELECT
...
CREATE OR REPLACE TEMP TABLE staging_tk_tw_profile_enriched_apoyo AS
SELECT DISTINCT
...
FROM bronze.tk_tw_profile_enriched_apoyo
WHERE id IS NOT NULL;

CREATE TABLE IF NOT EXISTS silver.tk_tw_profile_enriched_apoyo AS
SELECT * FROM staging_tk_tw_profile_enriched_apoyo WHERE 1 = 0;

MERGE INTO silver.tk_tw_profile_enriched_apoyo AS t
USING staging_tk_tw_profile_enriched_apoyo AS s
...
CREATE OR REPLACE TEMP TABLE staging_tk_tw_profile_post_apoyo AS
SELECT DISTINCT
  ...
  'twikit' AS source,
  current_timestamp AS ingested_at
FROM bronze.tk_tw_profile_post_apoyo
WHERE id IS NOT NULL;

CREATE TABLE IF NOT EXISTS silver.tk_tw_profile_post_apoyo AS
SELECT * FROM staging_tk_tw_profile_post_apoyo WHERE 1 = 0;

MERGE INTO silver.tk_tw_profile_post_apoyo AS t
USING staging_tk_tw_profile_post_apoyo AS s
...
-- ---------------------------------------------------------------------------
-- Silver: follow edges
-- filename like .../profiles/apoyo/<slug>_YYYYMMDD/followers.jsonl
-- ---------------------------------------------------------------------------

CREATE OR REPLACE TABLE silver.tk_tw_follow_edge_apoyo AS
WITH raw AS (
  SELECT
  ...
  FROM bronze.tk_tw_follow_edge_raw_apoyo
  WHERE id IS NOT NULL
),
parsed AS (
  SELECT
    ...
    regexp_extract(path_norm, 'profiles/apoyo/([^/]+)_[0-9]{8}/', 1) AS profile_slug
  FROM raw r
),
joined AS (
  SELECT
    ...
    current_timestamp AS ingested_at
  FROM parsed p
  INNER JOIN silver.tk_tw_profile_enriched_apoyo e
