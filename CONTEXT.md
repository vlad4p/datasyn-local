...
| **Quack / datasyn-rlab** | Remote DuckDB warehouse (client). Attach alias `"datasyn-rlab"`. Non-`main` schemas need `FROM "datasyn-rlab".query('…')` or `db.py quack-sql`. Hybrid ingest: `run-sql --ingest --attach-quack`. |
| **quack-host** | Serve **local** `datasyn.duckdb` over Quack HTTP (`db.py quack-host`, default `quack:127.0.0.1:9495`). Holds the file lock — stop before local MCP/ingest. Skill: `host-quack`. |
